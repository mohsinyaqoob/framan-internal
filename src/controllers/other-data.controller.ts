...
export const getAllNationalities = async (req, res) => {
  try {
    ...
    res.status(200).json({
      ...
      userData: { status: 0, data: { nationalities } },
    });
  } catch (error) {
    res.status(200).json({
      token: req.body.token,
      userData: { status: 1, data: { message: error.message } },
    });
  }
};
...
export const getClientAccountTypes = async (req, res) => {
  try {
    ...
    res.status(200).json({
      token: req.body.token,
      userData: { status: 0, data: { clientAccountTypes } },
    });
  } catch (error) {
    res.status(200).json({
      token: req.body.token,
...
export const getAccountOpeningMechanisms = async (req, res) => {
  try {
    ...
    res
      ...
      .json({
        token: req.body.token,
        userData: { status: 0, data: { accountOpeningMechanisms } },
      });
  } catch (error) {
    res.status(200).json({
      token: req.body.token,
