...
import { addOne,updateOne,getAll,getOne} from "../models/final-channel.model";
import { isValidAddFinalChannel,isValidUpdateFinalChannel,isValidGetOneFinalChannel } from "../utils/validate.util";
  ...
  export const getFinalChannel = async (req, res) => {
    try {
      ...
      res.status(200).json({
        status: 0,
        data: {
            count: FinalChannelDetails.length,FinalChannelDetails,
        },
      });
    } catch (error) {
      console.log(error);
